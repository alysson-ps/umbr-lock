...
use super::types::{SExpr, Token, Value};
    ...
    I: ValueInput<'tokens, Token = Token<'src>, Span = SimpleSpan>,
{
    recursive(|expr| {
        ...
        let attr_value = select! {
            Token::LiteralString(v) => Value::String(v.replace("\"", "").to_string()),
            Token::Binding(v) => Value::Binding(v.to_string()),
            Token::Number(v) => Value::Number(v.parse().unwrap())
        }
        .labelled("Attr value");
...
pub fn parser(code: &str) -> Option<SExpr> {
    ...
    // let (result, err) = parse().parse(token_stream).into_output_errors();
