...
macro_rules! properties_check {
    (
        ...
        impl $enum_name {
            pub fn check_propreties(existing_properties: &[SlintProperty]) -> Anyresult<()> {
                ...
                if missing_properties.is_empty() {
                    ...
                    Err(UmbrError::Generic(missing_properties.join(", ")))
                }
            }
...
properties_check!(
    RequiredProperties,
    Password -> ("passwd", ValueType::String)
);
...
macro_rules! callbacks_check {
    (
        ...
        impl $enum_name {
            pub fn check_callbacks(existing_callbacks: &[String]) -> Anyresult<()> {
                ...
                if missing_callbacks.is_empty() {
                    ...
                    Err(UmbrError::Generic(missing_callbacks.join(", ")))
                }
            }
